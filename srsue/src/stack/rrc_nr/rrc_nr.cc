...
{
  srsran::console("RRC NR reconfiguration launched (No HO).\n");
...
bool rrc_nr::rrc_reconfiguration_ho(bool endc_release_and_add_r15, const asn1::rrc_nr::rrc_recfg_s& rrc_nr_reconf)
{
  srsran::console("RRC NR reconfiguration launched.\n");
  if (not conn_recfg_ho_proc.launch(reconf_initiator_t::mcg_srb1, endc_release_and_add_r15, rrc_nr_reconf)) {
    logger.error("Unable to launch NR RRC reconfiguration with Handover procedure");
    return false;
  } else {
    callback_list.add_proc(conn_recfg_ho_proc);
  }
  return true;
}
...
void rrc_nr::handle_rrc_reconfig(const rrc_recfg_s& reconfig)
{
  ...
  if (reconfig.crit_exts.rrc_recfg().secondary_cell_group.size() > 0){
    if (not conn_recfg_ho_proc.launch(nr, false, reconfig)) {
      logger.error("Unable to launch connection reconfiguration procedure (with HO)");
      return;
    }
    callback_list.add_proc(conn_recfg_ho_proc);
  } else {
